...
function createServerOptions(context: ExtensionContext): ServerOptions {
  ...
  let args: string[] = []
  let command = ""
  const windowsExecutablePath = context.asAbsolutePath("out/server/Nezaboodka.Nevod.LanguageServer.exe")
  const linuxExecutablePath = context.asAbsolutePath("out/server/Nezaboodka.Nevod.LanguageServer")
  const darwinExecutablePath = context.asAbsolutePath("out/server/Nezaboodka.Nevod.LanguageServer")
  const portableDllPath = context.asAbsolutePath("out/server/Nezaboodka.Nevod.LanguageServer.dll")
